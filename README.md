...
What happens then is discriminator, generator, and gan weights are collected when all three losses are "reasonable".  When an individual model's loss goes out of bounds, then the last collected weights for that particular model are replaced, leaving the other weights are they are, and the process moves forward.  Mode collapse is identified when the same set of weights are used multiple times with no divergence from mode collapse.  
